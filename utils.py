...
def correct_mol(x):
    ...
    for neighbor in neighbors:
        continue
        #print("Atom: ", neighbor.GetIdx(), neighbor.GetSymbol())
    while True:
        flag, atomid_valence = check_valency(mol)
        ...
        if flag:
            ...
            if len(atomid_valence) == 1:
                # Remove all bonds associated with noble gasses
                idx = atomid_valence[0]
    ...
    #Remove hydrogen atoms without neighbors
    atoms_to_remove = []
    ...
    components = rdmolops.GetMolFrags(mol, asMols=True)
    largest_component = max(components, key=lambda component: component.GetNumAtoms())
