...
class Attention(gnn.MessagePassing):
    ...
    def __init__(self, embed_dim, num_heads=8, dropout=0., bias=False,
        ...
        if symmetric:
            self.to_qk = spectral_norm(nn.Linear(embed_dim, embed_dim, bias=bias))
        else:
            self.to_qk = spectral_norm(nn.Linear(embed_dim, embed_dim * 2, bias=bias))
        self.to_v = spectral_norm(nn.Linear(embed_dim, embed_dim, bias=bias))
