...
class GraphTransformer(nn.Module):
    def __init__(self, in_size=9, num_class=1, d_model=64, num_heads=8,
                 dim_feedforward=512, dropout=0.0, num_layers=4,
                 batch_norm=False, abs_pe=False, abs_pe_dim=0,
                 gnn_type="graph", se="gnn", use_edge_attr=True, num_edge_features=3,
        ...
        if in_embed:
            ...
            self.embedding = spectral_norm(nn.Linear(in_features=in_size,
        ...
        if use_edge_attr:
            ...
            if edge_embed:
                ...
                self.embedding_edge = spectral_norm(nn.Linear(in_features=num_edge_features,
        ...
        if max_seq_len is None:
            self.classifier = nn.Sequential(
                ...
                nn.SiLU(True),
                spectral_norm(nn.Linear(d_model, num_class))
            )
        else:
            self.classifier = nn.ModuleList()
            for i in range(max_seq_len):
                self.classifier.append(spectral_norm(nn.Linear(d_model, num_class)))
